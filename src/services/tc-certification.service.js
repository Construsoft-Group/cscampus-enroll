// src/services/tc-certification.service.js
...
// === CONFIGURACIÓN CURSO TC CERTIFICATION ===
const COURSE_ID = 249;
const COURSE_NAME = "Certificación Trimble Connect";
const COURSE_LINK = `https://campus.construsoft.com/user/index.php?id=${COURSE_ID}`;
const GROUP_ID = 0; // TODO: Configurar el ID del grupo correcto

export const customerEnrollmentReq_TCCertification = async (req, res) => {
  const now = new Date();
  const formData = new formidable.IncomingForm();

  formData.parse(req, async (error, fields) => {
    if (error) {
      console.error('[TC-CERTIFICATION][FORM ERROR]', error);
    ...
    await sendInternalEmail(newUser, 'Formulario TC Certification');
    ...
    // === Agregar al grupo si GROUP_ID está configurado ===
    if (GROUP_ID > 0) {
    ...
    const dataResponse = {
      ...
      message: 'Estás a un paso de comenzar tu certificación en Trimble Connect.',
      link: {
        url: COURSE_LINK,
...
export const renderCourseForm_TCCertification = async (_req, res) => {
  res.render('forms/tc-certification/customer-enroll');
};
